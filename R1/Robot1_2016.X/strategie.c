...
//        passe_part(1000, 1000, MARCHE_ARRIERE, 50, DEBUT_TRAJECTOIRE);//A rajouter si on commence par les poissons coté violet
        if(COULEUR == VERT)   //Signifie VIOLET
...
//                        passe_part(310, 1900, MARCHE_AVANT, 50, DEBUT_DEPLACEMENT);
    ...
    //                        orienter(180,100);//Réoriente bien parallèle à la bordure
                        ...
                        {//on recule un peu
                            ...
                            rejoindre(610, 150, MARCHE_ARRIERE, 60);//On se place près du bac
...
//                            rejoindre(790, 150, MARCHE_ARRIERE, 50);//On se déplace dans tout le bac (pour récuperer les poissons)
                            rejoindre(785, 150, MARCHE_ARRIERE, 50);//On se déplace dans tout le bac (pour récuperer les poissons)
...
//                            while(read_data(ROT_FILET, LIRE_POSITION_ACTU) < 365);//On attend que le dernière AX12 est fini de bouger
                        ...
                        {//on recule un peu
                            ...
                            rejoindre(645, 160, MARCHE_AVANT, 60);//On se place près du bac
                            orienter(0,100);//Réoriente bien parallèle à la bordure
                            //orienter(180,100);//Réoriente bien parallèle à la bordure
                        ...
                        {//On avance dans le bac et on remonte le filet (on gère les cas d'impossibilité de remonter)
                            ...
                            rejoindre(807, 160, MARCHE_AVANT, 50);//On se déplace dans tout le bac (pour récuperer les poissons)
                            ...
                            if(fin_strategie_cause_evitement == 0)//Si il n'y a pas eu la backup strat
                            {
                                ...
                                while(read_data(DEPLOIMENT_BRAS_FILET, LIRE_POSITION_ACTU) < 585 && fin_strategie_cause_evitement == 0)
                                {
                                    if((COMPTEUR_TEMPS_MATCH - temporisation) > 2)
                                        angle_AX12(DEPLOIMENT_BRAS_FILET, 600, 600, SANS_ATTENTE);   //Position intermédiaire (pour passer la barre du filet)
                                    if((COMPTEUR_TEMPS_MATCH - temporisation) > 4)
                                        rejoindre(640, 150, MARCHE_AVANT, 40);//On recule un peu pour débloquer
                                }
